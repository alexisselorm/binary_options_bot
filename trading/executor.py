...
class TradeExecutor:
    ...
    async def run(self):
        ...
        while True:
            try:
                # Only fetch candles if we're not relying solely on external signals
                if not getattr(self.cfg, 'enable_external_signals', False):
                    ...
                    # When external signals are enabled, we just wait for them
